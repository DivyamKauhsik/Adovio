...
// Works in Claude preview AND Vercel deployment
const getApiKey = () => {
...
if (score >= 4.2) return { label:"Strong", color:T.sage, bg:T.lime, desc:"Your organisation demonstrates strong readiness fundamentals." };
if (score >= 3.2) return { label:"Moderate", color:T.gold, bg:T.goldL, desc:"There is a reasonable foundation, but important gaps remain." };
if (score >= 2.2) return { label:"At Risk", color:"#C0580A", bg:"#FEF0E6",desc:"Significant readiness gaps exist that could derail success." };
return { label:"Critical", color:T.red, bg:T.redL, desc:"Immediate action is required before proceeding with this change." };
}
...
return (
<span style={{ display:"inline-block", padding:"3px 12px", borderRadius:100, background:bg, color, fontSize:11, fontWeight:600, letterSpacing:.8, textTransform:"uppercase" }}>
{children}
</span>
);
}
...
width:full?"100%":"auto", boxShadow:disabled?"none":"0 2px 8px rgba(27,67,50,.25)",
letterSpacing:.3,
}}
...
return <div style={{ fontSize:11, fontWeight:600, letterSpacing:1, textTransform:"uppercase", color:T.muted, marginBottom:6 }}>{children}</div>;
}
...
background:i<=current?T.forest:T.white,
border:`2px solid ${i<=current?T.forest:T.border}`,
display:"flex", alignItems:"center", justifyContent:"center",
fontSize:12, fontWeight:600, color:i<=current?T.white:T.muted, transition:"all .3s",
}}>{i<current?"✓":i+1}</div>
...
<Badge color={T.lime} bg="rgba(255,255,255,.12)">Free · No Login Required</Badge>
</div>
...
<button onClick={()=>onStart("change")} style={{
padding:"14px 32px", borderRadius:8, fontSize:15, fontWeight:600,
background:T.white, color:T.forest, border:"none", cursor:"pointer",
boxShadow:"0 4px 16px rgba(0,0,0,.2)", transition:"all .2s",
}}
onMouseEnter={e=>e.currentTarget.style.transform="translateY(-2px)"}
onMouseLeave={e=>e.currentTarget.style.transform="translateY(0)"}
>Change Readiness →</button>
<button onClick={()=>onStart("ai")} style={{
padding:"14px 32px", borderRadius:8, fontSize:15, fontWeight:600,
background:"transparent", color:T.white, border:"1.5px solid rgba(255,255,255,.4)",
...
<div>
<Label>Briefly describe the change or initiative<span style={{color:T.red}}> *</span></Label>
<textarea value={context.changeDesc} onChange={e=>setContext(c=>({...c,changeDesc:e.target.value}))}
placeholder={type==="change"?"e.g. We are implementing a new ERP system across 3 business units...":"e.g. We are rolling out Microsoft Copilot to 500 employees in Q3..."}
...
return (
<button key={s.v} onClick={()=>setAnswers(a=>({...a,[q.id]:s.v}))} style={{
padding:"7px 14px", borderRadius:6, fontSize:12, fontWeight:500,
background:active?accent:T.white, color:active?T.white:T.mid,
border:`1.5px solid ${active?accent:T.border}`, transition:"all .15s",
}}
...
<p style={{ color:T.mid, fontSize:14, lineHeight:1.7 }}>Your personalised AI report will be displayed immediately. We'll also keep a copy for your records.</p>
</div>
...
useEffect(()=>{ const t=setInterval(()=>setStep(s=>Math.min(s+1,steps.length-1)),1600); return()=>clearInterval(t); },[]);
const accent = type==="change"?T.forest:T.blue;
return (
...
<path d="M 14 70 A 56 56 0 0 1 126 70" fill="none" stroke={T.border} strokeWidth={10} strokeLinecap="round"/>
<path d="M 14 70 A 56 56 0 0 1 126 70" fill="none" stroke={lv.color} strokeWidth={10}
strokeDasharray={`${dash} ${circumference}`} strokeLinecap="round"
style={{ transition:"stroke-dasharray 1.2s ease", filter:`drop-shadow(0 0 6px ${lv.color}60)` }}/>
<text x={70} y={66} textAnchor="middle" fill={T.ink} style={{ fontSize:22, fontWeight:700, fontFamily:"'Playfair Display',serif" }}>{score.toFixed(1)}</text>
<text x={70} y={82} textAnchor="middle" fill={T.muted} style={{ fontSize:10, fontFamily:"'Inter',sans-serif" }}>out of 5.0</text>
</svg>
<div style={{ display:"inline-block", marginTop:4, padding:"4px 16px", borderRadius:100, background:lv.bg, color:lv.color, fontSize:13, fontWeight:700 }}>{lv.label} Readiness</div>
</div>
);
}
...
const overallScore = avgScore(answers, questions);
const lv = getLevel(overallScore);
...
<Steps current={3}/>
<div style={{ background:`linear-gradient(135deg,${T.forest} 0%,#0D2B1F 100%)`, borderRadius:16, padding:"40px 40px 36px", marginBottom:32, color:T.white, position:"relative", overflow:"hidden" }}>
...
<h1 className="serif" style={{ fontSize:32, fontWeight:700, marginTop:14, marginBottom:4, letterSpacing:-.5 }}>{context.orgName}</h1>
<p style={{ color:"rgba(255,255,255,.6)", fontSize:13 }}>Prepared for {respondent.name} · {respondent.email} · {formatDate()}</p>
{context.changeDesc && <p style={{ color:"rgba(255,255,255,.5)", fontSize:12, marginTop:8, fontStyle:"italic", maxWidth:480 }}>"{context.changeDesc}"</p>}
{(context.industry||context.size) && (
<div style={{ marginTop:16, display:"flex", gap:12, flexWrap:"wrap" }}>
{context.industry && <span style={{ fontSize:11, padding:"3px 10px", borderRadius:100, background:"rgba(255,255,255,.12)", color:"rgba(255,255,255,.7)" }}>{context.industry}</span>}
{context.size && <span style={{ fontSize:11, padding:"3px 10px", borderRadius:100, background:"rgba(255,255,255,.12)", color:"rgba(255,255,255,.7)" }}>{context.size} employees</span>}
<span style={{ fontSize:11, padding:"3px 10px", borderRadius:100, background:"rgba(255,255,255,.12)", color:"rgba(255,255,255,.7)" }}>{respondent.role}</span>
</div>
)}
</div>
</div>

<div style={{ background:T.white, borderRadius:12, border:`1px solid ${T.border}`, padding:"36px", marginBottom:24, display:"grid", gridTemplateColumns:"auto 1fr", gap:36, alignItems:"center", boxShadow:"0 2px 12px rgba(0,0,0,.05)" }}>
<ScoreGauge score={overallScore}/>
<div>
<div style={{ fontSize:13, fontWeight:600, color:T.muted, letterSpacing:.5, textTransform:"uppercase", marginBottom:6 }}>Overall Score</div>
<div style={{ fontSize:28, fontWeight:700, fontFamily:"'Playfair Display',serif", color:lv.color, marginBottom:8 }}>{overallScore.toFixed(2)} / 5.00</div>
<p style={{ fontSize:14, color:T.mid, lineHeight:1.65 }}>{lv.desc}</p>
<div style={{ marginTop:16, display:"flex", gap:8, flexWrap:"wrap" }}>
{[[lv.label,"readiness level"],[`${questions.length} dimensions`,"assessed"],[`${scored.filter(q=>q.score>=4).length} strengths`,"identified"]].map(([v,l])=>(
<div key={l} style={{ padding:"6px 14px", borderRadius:8, background:T.off, border:`1px solid ${T.border}` }}>
...
<h3 className="serif" style={{ fontSize:20, fontWeight:700, marginBottom:24, letterSpacing:-.2 }}>Dimension Breakdown</h3>
<div style={{ display:"flex", flexDirection:"column", gap:12 }}>
{scored.map(q=>{ const lv2=getLevel(q.score); return (
<div key={q.id} style={{ display:"grid", gridTemplateColumns:"160px 1fr 40px", alignItems:"center", gap:14 }}>
<div style={{ fontSize:13, color:T.body, fontWeight:500 }}>{q.cat}</div>
<div style={{ height:8, background:T.off, borderRadius:4, overflow:"hidden", border:`1px solid ${T.border}` }}>
<div style={{ height:"100%", width:`${((q.score-1)/4)*100}%`, borderRadius:4, background:lv2.color, transition:"width 1s ease" }}/>
</div>
<div style={{ fontSize:13, fontWeight:600, color:lv2.color, textAlign:"right" }}>{q.score}.0</div>
</div>
);})}
</div>
</div>
...
{[["✅ Top Strengths",strongest,T.sage,T.lime,"Leverage these as anchors for the change."],["⚠️ Priority Risks",weakest,"#C0580A","#FEF0E6","Address these before they derail momentum."]].map(([heading,items,color,bg,sub])=>(
...
<div style={{ fontWeight:700, fontSize:14, color, marginBottom:4 }}>{heading}</div>
...
<div style={{ fontSize:11, color:T.mid, lineHeight:1.5 }}>{q.text.length>80?q.text.substring(0,80)+"…":q.text}</div>
</div>
))}
</div>
))}
...
<div key={i} style={{ background:T.white, borderRadius:12, padding:"28px 32px", border:`1px solid ${T.border}`, borderLeft:`4px solid ${accent}`, boxShadow:"0 2px 8px rgba(0,0,0,.04)", marginBottom:16 }}>
<h4 className="serif" style={{ fontSize:17, fontWeight:700, color:T.ink, marginBottom:12 }}>{s.heading}</h4>
<div style={{ fontSize:14, color:T.mid, lineHeight:1.8, whiteSpace:"pre-wrap" }}>{s.body}</div>
</div>
)) : (
<div style={{ background:T.white, borderRadius:12, padding:"28px 32px", border:`1px solid ${T.border}`, borderLeft:`4px solid ${accent}` }}>
<div style={{ fontSize:14, color:T.mid, lineHeight:1.8, whiteSpace:"pre-wrap" }}>{aiText}</div>
</div>
)}
</div>

<div style={{ background:T.lime, borderRadius:12, border:`1px solid ${T.forest}30`, padding:"24px 28px", marginBottom:32, display:"flex", gap:16, alignItems:"center" }}>
<div style={{ fontSize:28 }}>✉️</div>
<div>
<div style={{ fontWeight:600, fontSize:14, color:T.forest, marginBottom:2 }}>Report saved to your inbox</div>
<div style={{ fontSize:13, color:T.mid }}>A copy of this report will be sent to <strong>{respondent?.email}</strong></div>
</div>
</div>
...
<h3 className="serif" style={{ fontSize:22, fontWeight:700, marginBottom:8 }}>{isChange?"Also take the AI Adoption Readiness Assessment":"Also take the Change Readiness Assessment"}</h3>
<p style={{ color:T.mid, fontSize:14, marginBottom:24 }}>Get the full picture of your organisation's transformation readiness.</p>
<div style={{ display:"flex", gap:14, justifyContent:"center", flexWrap:"wrap" }}>
...
const resetFor = (t) => {
setType(t); setAnswers({}); setContext({ orgName:"", role:"", changeDesc:"", industry:"", size:"" });
...
? `You are a senior change management consultant with 20+ years of experience. Write with authority, empathy, and precision. Reference actual scores and categories. Avoid generic advice. Use ## headings. Write in clear professional English.`
: `You are an expert in AI adoption and organisational change. You deeply understand both human and technical dimensions of AI transformation. Reference actual scores. Avoid generic advice. Use ## headings. Write in clear professional English.`;
...
## Executive Summary
3–4 sentences. Direct and honest overview.

## Key Findings
3–4 most important insights referencing specific scores.

## Critical Risk Areas
Focus on lowest-scoring dimensions and real-world consequences.

## Recommended Actions
4–5 concrete prioritised actions specific to these scores.

## What Good Looks Like
Tangible picture of success if recommendations are followed.`;
...
onMouseLeave={e=>e.currentTarget.style.background="transparent"}
>Change Readiness</button>
<PrimaryBtn onClick={()=>resetFor("ai")}>AI Readiness</PrimaryBtn>
</div>
</nav>
